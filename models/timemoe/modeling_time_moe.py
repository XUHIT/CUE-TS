...
class TimeMoeSparseExpertsLayer(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        # print(f"routing_weights shape: {routing_weights.shape}, dtype: {routing_weights.dtype}")
        # print('mean_weights:', routing_weights.mean(dim=0, keepdim=True))
...
class TimeMoeForPrediction(TimeMoePreTrainedModel):
    ...
    def __init__(self, config: TimeMoeConfig):
        ...
        self.schedule_strategy = "reverse_greedy" 
    ...
    def forward(
        ...
        self.visual_logits= outputs.router_logits##
        ...
        if labels is not None:
            ...
            main_loss = ar_loss / len(self.config.horizon_lengths)   
            loss       = main_loss                                  
            ...
            if self.apply_aux_loss:
                ...
                aux_loss = self.router_aux_loss_factor * temporal_aux_loss.to(loss.device)
                loss =loss + aux_loss
        else:
        ...
        if labels is not None:
            setattr(outputs, "main_loss", main_loss)
            setattr(outputs, "aux_loss",  aux_loss)

        return outputs
    
    def _visual_logits(self):
        return self.visual_logits
